...
social1 <- readRDS(here::here('Data/Processed/TransitionLayers/socialtop5_YNP_NBR.rds'))
biophys <- readRDS(here::here('Data/Processed/TransitionLayers/biophystop5_YNP_NBR.rds'))
...
flextable::save_as_docx(current.dens, path = here::here("plots/table2_YNP_NBR.docx"))
...
p3 <- ggplot(data = filter(dist.sum, name == "abase"), mapping = aes(x = ID, 
                                                                     y = mn, 
                                                                     color = costdist)) +
...
ggsave('plots/fig5_YNP_NBR.png', p3)
