...
social1 <- readRDS(here::here('Data/Processed/TransitionLayers/socialtop5_YNP_FP.rds'))
biophys <- readRDS(here::here('Data/Processed/TransitionLayers/biophystop5_YNP_FP.rds'))
...
flextable::save_as_docx(current.dens, path = here::here("plots/table2_YNP_FP.docx"))
...
p2 <- ggplot(data = filter(dist.sum, name == "abase"), mapping = aes(x = ID, 
                                                                     y = mn, 
                                                                     color = costdist)) +
...
combined
ggsave('plots/fig5_full.png', combined)
