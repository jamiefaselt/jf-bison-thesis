...
eucdist <- accCost(euclidean.tr, pa.cents)

#extract values for last 50km
euc.dist <- st_distance(as(pa.cents, "sf"), as(pa.cents, "sf"))
units(euc.dist) <- NULL
...
soc.costdist1 <- accCost(social.tr1, pa.cents)
bio.costdist <- accCost(biophys.tr, pa.cents)
...
names(distance.extract) <- c("s1", "s2", "s3","b1", "b2", "b3")
...
flextable::save_as_docx(current.dens, path = here::here("plots/table2_mst_draft2.docx"))
